...
lr_auc_fe <- 
  lr_res_fe %>% 
  collect_predictions(parameters = lr_best_fe) %>% 
...
rf_auc_fe <- 
  rf_res_fe %>% 
  collect_predictions(parameters = rf_best_fe) %>% 
  roc_curve(INFEC_after_diag, .pred_1) %>% 
...
svm_auc_fe <- 
  svm_res_fe %>% 
  collect_predictions(parameters = svm_best_fe) %>% 
  roc_curve(INFEC_after_diag, .pred_1) %>% 
  mutate(model = "SVM")
...
```{r}
svm_r_res_fe = readRDS("svm_r_res_fe")
```
We can print the metrics from the model training using the collect_metrics command.
```{r}
svm_r_res_fe %>% 
...
svm_r_auc_fe <- 
  svm_r_res_fe %>% 
  collect_predictions(parameters = svm_r_best_fe) %>% 
  roc_curve(INFEC_after_diag, .pred_1) %>% 
  mutate(model = "SVM radial")

autoplot(svm_r_auc_fe)
```

## extrem gradient boost
```{r}
xgb_res_fe = readRDS("xgb_res_fe")
...
xgb_auc_fe <- 
  xgb_res_fe %>% 
  collect_predictions(parameters = xgb_best_fe) %>% 
  roc_curve(INFEC_after_diag, .pred_1) %>% 
...
mlp_auc_fe <- 
  mlp_res_fe %>% 
  collect_predictions(parameters = mlp_best_fe) %>% 
  roc_curve(INFEC_after_diag, .pred_1) %>% 
...
nb_auc_fe <- 
  nb_res_fe %>% 
  collect_predictions(parameters = nb_best_fe) %>% 
  roc_curve(INFEC_after_diag, .pred_1) %>% 
...
bag_auc_fe <- 
  bag_res_fe %>% 
  collect_predictions(parameters = bag_best_fe) %>% 
  roc_curve(INFEC_after_diag, .pred_1) %>% 
...
dec_auc_fe <- 
  dec_res_fe %>% 
  collect_predictions(parameters = dec_best_fe) %>% 
  roc_curve(INFEC_after_diag, .pred_1) %>% 
...
<!-- saveRDS(mars_res_fe, "/users/home/candwei/mars_res_fe") -->
<!-- ``` -->
...
<!-- mars_auc_fe <-  -->
<!--   mars_res_fe %>%  -->
<!--   collect_predictions(parameters = mars_best_fe) %>%  -->
<!--   roc_curve(INFEC_after_diag, .pred_1) %>%  -->
<!--   mutate(model = "Multivariate adaptive regression splines") -->

<!-- autoplot(mars_auc_fe) -->
<!-- ``` -->
...
lr_final_mod_fe <- logistic_reg(penalty = lr_best_fe$penalty, mixture = lr_best_fe$mixture) %>% 
  set_engine("glmnet")

# Create final workflow
lr_final_workflow_fe <- workflow() %>% 
 add_model(lr_final_mod_fe) %>% 
  add_recipe(cll_rec)

# Last fit
lr_last_fit_fe <- last_fit(lr_final_workflow_fe, split = df_split, metrics = multi_metric)
lr_last_fit_fe %>% collect_metrics() %>% select(1,3)
```
...
rf_final_mod_fe <- 
  rand_forest(mtry = rf_best_fe$mtry, min_n = rf_best_fe$min_n, trees = rf_best_fe$trees) %>% 
  set_engine("ranger", importance = "impurity") %>% 
  set_mode("classification")

# Create final workflow
rf_final_workflow_fe <- workflow() %>% 
 add_model(rf_final_mod_fe) %>% 
  add_recipe(cll_rec)

# Last fit
rf_last_fit_fe <- last_fit(rf_final_workflow_fe, split = df_split, metrics = multi_metric)
rf_last_fit_fe %>% collect_metrics() %>% select (1,3)
```
...
# In order to extract the variable importance we need to specify it in set_engine
svm_final_mod_fe <- 
  svm_poly(cost = svm_best_fe$cost, degree = svm_best_fe$degree) %>% 
  set_engine("kernlab") %>% 
  set_mode("classification")

# Create final workflow
svm_final_workflow_fe <- workflow() %>% 
  add_model(svm_final_mod_fe) %>% 
  add_recipe(cll_rec)

# Last fit
svm_last_fit_fe <- last_fit(svm_final_workflow_fe, split = df_split, metrics = multi_metric)
svm_last_fit_fe %>% collect_metrics() %>% select(1,3)
```
...
# In order to extract the variable importance we need to specify it in set_engine
svm_r_final_mod_fe <- 
  svm_rbf(cost = svm_r_best_fe$cost, rbf_sigma = svm_r_best_fe$rbf_sigma) %>% 
  set_engine("kernlab") %>% 
  set_mode("classification")

# Create final workflow
svm_r_final_workflow_fe <- workflow() %>% 
 add_model(svm_r_final_mod_fe) %>% 
  add_recipe(cll_rec)

# Last fit
svm_r_last_fit_fe <- last_fit(svm_r_final_workflow_fe, split = df_split, metrics = multi_metric)
svm_r_last_fit_fe %>% collect_metrics() %>% select(1,3)
```
...
# In order to extract the variable importance we need to specify it in set_engine
xgb_final_mod_fe <- 
  boost_tree(min_n = xgb_best_fe$min_n,tree_depth = xgb_best_fe$tree_depth, learn_rate = xgb_best_fe$learn_rate,loss_reduction = xgb_best_fe$loss_reduction, trees=xgb_best_fe$trees) %>% 
  set_engine("xgboost") %>% 
  set_mode("classification")

# Create final workflow
xgb_final_workflow_fe <- workflow() %>% 
 add_model(xgb_final_mod_fe) %>% 
  add_recipe(cll_rec)

# Last fit
xgb_last_fit_fe <- last_fit(xgb_final_workflow_fe, split = df_split, metrics = multi_metric)
xgb_last_fit_fe %>% collect_metrics() %>% select(1,3)
```
...
# In order to extract the variable importance we need to specify it in set_engine
mlp_final_mod_fe <- 
  mlp(hidden_units = mlp_best_fe$hidden_units, penalty = mlp_best_fe$penalty, epochs = mlp_best_fe$epochs) %>% 
  set_engine("nnet", trace=0,MaxNWts=10000) %>% 
  set_mode("classification")

# Create final workflow
mlp_final_workflow_fe <- workflow() %>% 
 add_model(mlp_final_mod_fe) %>% 
  add_recipe(cll_rec)

# Last fit
mlp_last_fit_fe <- last_fit(mlp_final_workflow_fe, split = df_split, metrics = multi_metric)
mlp_last_fit_fe %>% collect_metrics() %>% select(1,3)
```
...
bag_final_mod_fe <- 
   baguette::bag_tree(
           min_n = bag_best_fe$min_n,
           class_cost = bag_best_fe$class_cost)%>% 
  set_engine("C5.0") %>% 
  set_mode("classification")


# Create final workflow
bag_final_workflow_fe <- workflow() %>% 
 add_model(bag_final_mod_fe) %>% 
  add_recipe(cll_rec)

# Last fit
bag_last_fit_fe <- last_fit(bag_final_workflow_fe, split = df_split, metrics = multi_metric)
bag_last_fit_fe %>% collect_metrics() %>% select(1,3)
```
...
# In order to extract the variable importance we need to specify it in set_engine
nb_final_mod_fe <- 
  naive_Bayes(smoothness = nb_best_fe$smoothness, Laplace = nb_best_fe$Laplace) %>% 
  set_engine("naivebayes") %>% 
  set_mode("classification")


# Create final workflow
nb_final_workflow_fe <- workflow() %>% 
 add_model(nb_final_mod_fe) %>% 
  add_recipe(cll_rec)

# Last fit
nb_last_fit_fe <- last_fit(nb_final_workflow_fe, split = df_split, metrics = multi_metric)
nb_last_fit_fe %>% collect_metrics() %>% select(1,3)
```
...
dec_final_mod_fe <- 
  decision_tree(
    min_n = dec_best_fe$min_n,
    cost_complexity = dec_best_fe$cost_complexity,
    tree_depth = dec_best_fe$tree_depth) %>% 
  set_engine("rpart") %>% 
  set_mode("classification")

# Create final workflow
dec_final_workflow_fe <- workflow() %>% 
 add_model(dec_final_mod_fe) %>% 
  add_recipe(cll_rec)

# Last fit
dec_last_fit_fe <- last_fit(dec_final_workflow_fe, split = df_split, metrics = multi_metric)
dec_last_fit_fe %>% collect_metrics() %>% select(1,3)
```
...
<!-- mars_final_mod_fe <-  -->
<!--   mars( -->
<!--     num_terms = mars_best_fe$num_terms, -->
<!--     prune_method = mars_best_fe$prune_method, -->
<!--     prod_degree = mars_best_fe$prod_degree) %>%  -->
<!--   set_engine("earth") %>%  -->
<!--   set_mode("classification") -->

<!-- # Create final workflow -->
<!-- mars_final_workflow_fe <- workflow() %>%  -->
<!--  add_model(mars_final_mod_fe) %>%  -->
<!--   add_recipe(cll_rec) -->

<!-- # Last fit -->
<!-- mars_last_fit_fe <- last_fit(mars_final_workflow_fe, split = df_split) -->
<!-- mars_last_fit_fe %>% collect_metrics() -->
<!-- ``` -->

# Create stack
## Create stack
```{r}
cll_st_fe = readRDS("cll_st_fe")
```
...
```{r}
autoplot(cll_st_fe, type = "members")
```

## Look at penalty 
```{r}
autoplot(cll_st_fe, type="weights")
```

## Check parameters
```{r}
collect_parameters(cll_st_fe,"lr_res_fe")
```
...
cll_pred_fe <-
  df_test %>%
  bind_cols(predict(cll_st_fe,.,type = "prob"))
...
```{r}
map_dfr(
  setNames(colnames(cll_pred_fe),colnames(cll_pred_fe)),
  ~mean(cll_pred_fe$INFEC_after_diag == pull(cll_pred_fe, .x))
) %>%
...
```{r}
lr_final_fe = lr_last_fit_fe %>%
  pluck(".workflow", 1)


df_lr_fe = bind_cols(
  df_test,
  predict(lr_final_fe, df_test),
  predict(lr_final_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_lr_fe$bs =  (df_lr_fe$.pred_1 - df_lr_fe$INFEC_after_diag_num)^2
mean(df_lr_fe$bs)
```

## create calibration plot
```{r}
lr_cp = classifierplots::calibration_plot(df_lr_fe$INFEC_after_diag, df_lr_fe$.pred_1)
lr_cp
```
## RF
### extract models from last fit
```{r}
rf_final_fe = rf_last_fit_fe %>%
  pluck(".workflow", 1)


df_rf_fe = bind_cols(
  df_test,
  predict(rf_final_fe, df_test),
  predict(rf_final_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_rf_fe$bs =  (df_rf_fe$.pred_1 - df_rf_fe$INFEC_after_diag_num)^2
mean(df_rf_fe$bs)
```

## create calibration plot
```{r}
rf_cp = classifierplots::calibration_plot(df_rf_fe$INFEC_after_diag, df_rf_fe$.pred_1)
rf_cp
```
...
### extract models from last fit
```{r}
svm_final_fe = svm_last_fit_fe %>%
  pluck(".workflow", 1)


df_svm_fe = bind_cols(
  df_test,
  predict(svm_final_fe, df_test),
  predict(svm_final_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_svm_fe$bs =  (df_svm_fe$.pred_1 - df_svm_fe$INFEC_after_diag_num)^2
mean(df_svm_fe$bs)
```

## create calibration plot
```{r}
svm_cp = classifierplots::calibration_plot(df_svm_fe$INFEC_after_diag, df_svm_fe$.pred_1)
svm_cp
```
## SVM with radient kernel basis
### extract models from last fit
```{r}
svm_r_final_fe = svm_r_last_fit_fe %>%
  pluck(".workflow", 1)


df_svm_r_fe = bind_cols(
  df_test,
  predict(svm_r_final_fe, df_test),
  predict(svm_r_final_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_svm_r_fe$bs =  (df_svm_r_fe$.pred_1 - df_svm_r_fe$INFEC_after_diag_num)^2
mean(df_svm_r_fe$bs)
```

## create calibration plot
```{r}
svm_r_cp = classifierplots::calibration_plot(df_svm_r_fe$INFEC_after_diag, df_svm_r_fe$.pred_1)
svm_r_cp
```
...
### extract models from last fit
```{r}
xgb_final_fe = xgb_last_fit_fe %>%
  pluck(".workflow", 1)


df_xgb_fe = bind_cols(
  df_test,
  predict(xgb_final_fe, df_test),
  predict(xgb_final_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_xgb_fe$bs =  (df_xgb_fe$.pred_1 - df_xgb_fe$INFEC_after_diag_num)^2
mean(df_xgb_fe$bs)
```

## create calibration plot
```{r}
xgb_cp = classifierplots::calibration_plot(df_xgb_fe$INFEC_after_diag, df_xgb_fe$.pred_1)
xgb_cp
```
...
### extract models from last fit
```{r}
nb_final_fe = nb_last_fit_fe %>%
  pluck(".workflow", 1)


df_nb_fe = bind_cols(
  df_test,
  predict(nb_final_fe, df_test),
  predict(nb_final_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_nb_fe$bs =  (df_nb_fe$.pred_1 - df_nb_fe$INFEC_after_diag_num)^2
mean(df_nb_fe$bs)
```

## create calibration plot
```{r}
nb_cp = classifierplots::calibration_plot(df_nb_fe$INFEC_after_diag, df_nb_fe$.pred_1)
nb_cp
```
...
### extract models from last fit
```{r}
mlp_final_fe = mlp_last_fit_fe %>%
  pluck(".workflow", 1)


df_mlp_fe = bind_cols(
  df_test,
  predict(mlp_final_fe, df_test),
  predict(mlp_final_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_mlp_fe$bs =  (df_mlp_fe$.pred_1 - df_mlp_fe$INFEC_after_diag_num)^2
mean(df_mlp_fe$bs)
```

## create calibration plot
```{r}
mlp_cp = classifierplots::calibration_plot(df_mlp_fe$INFEC_after_diag, df_mlp_fe$.pred_1)
mlp_cp
```
...
### extract models from last fit
```{r}
bag_final_fe = bag_last_fit_fe %>%
  pluck(".workflow", 1)


df_bag_fe = bind_cols(
  df_test,
  predict(bag_final_fe, df_test),
  predict(bag_final_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_bag_fe$bs =  (df_bag_fe$.pred_1 - df_bag_fe$INFEC_after_diag_num)^2
mean(df_bag_fe$bs)
```

## create calibration plot
```{r}
bag_cp = classifierplots::calibration_plot(df_bag_fe$INFEC_after_diag, df_bag_fe$.pred_1)
bag_cp
```
...
### extract models from last fit
```{r}
dec_final_fe = dec_last_fit_fe %>%
  pluck(".workflow", 1)


df_dec_fe = bind_cols(
  df_test,
  predict(dec_final_fe, df_test),
  predict(dec_final_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_dec_fe$bs =  (df_dec_fe$.pred_1 - df_dec_fe$INFEC_after_diag_num)^2
mean(df_dec_fe$bs)
```

## create calibration plot
```{r}
dec_cp = classifierplots::calibration_plot(df_dec_fe$INFEC_after_diag, df_dec_fe$.pred_1)
dec_cp
```
...
<!-- ### extract models from last fit -->
<!-- ```{r} -->
<!-- mars_final_fe = mars_last_fit_fe %>% -->
<!--   pluck(".workflow", 1) -->


<!-- df_mars_fe = bind_cols( -->
<!--   df_test, -->
<!--   predict(mars_final_fe, df_test), -->
<!--   predict(mars_final_fe, df_test, type="prob") -->
<!-- ) -->
<!-- ``` -->

<!-- ## calculate Brier score -->
<!-- ```{r} -->
<!-- df_mars_fe$bs =  (df_mars_fe$.pred_1 - df_mars_fe$INFEC_after_diag_num)^2 -->
<!-- mean(df_mars_fe$bs) -->
<!-- ``` -->

<!-- ## create calibration plot -->
<!-- ```{r} -->
<!-- mars_cp = classifierplots::calibration_plot(df_mars_fe$INFEC_after_diag, df_mars_fe$.pred_1) -->
<!-- mars_cp -->
<!-- ``` -->
...
### extract models from last fit
```{r}
df_cll_st_fe = bind_cols(
  df_test,
  predict(cll_st_fe, df_test),
  predict(cll_st_fe, df_test, type="prob")
)
```

## calculate Brier score
```{r}
df_cll_st_fe$bs =  (df_cll_st_fe$.pred_1 - df_cll_st_fe$INFEC_after_diag_num)^2
mean(df_cll_st_fe$bs)
```

## create calibration plot
```{r}
cll_st_cp = classifierplots::calibration_plot(df_cll_st_fe$INFEC_after_diag, df_cll_st_fe$.pred_1)
cll_st_cp
```

lr_clp = classifierplots::classifierplots(df_lr_fe$INFEC_after_diag, df_lr_fe$.pred_1)
lr_clp

dir.create("plot_feli_lr")
classifierplots::classifierplots_folder(
  test.y=df_lr_fe$INFEC_after_diag,
  pred.prob = df_lr_fe$.pred_1,
  folder = "plot_feli_lr",
  height = 250,
  width = 250
)

rf_clp = classifierplots::classifierplots(df_rf_fe$INFEC_after_diag, df_rf_fe$.pred_1)
rf_clp

dir.create("plot_feli_rf")
classifierplots::classifierplots_folder(
  test.y=df_rf_fe$INFEC_after_diag,
  pred.prob = df_rf_fe$.pred_1,
  folder = "plot_feli_rf",
  height = 250,
  width = 250
)

svm_clp = classifierplots::classifierplots(df_svm_fe$INFEC_after_diag, df_svm_fe$.pred_1)
svm_clp

dir.create("plot_feli_svm")
classifierplots::classifierplots_folder(
  test.y=df_svm_fe$INFEC_after_diag,
  pred.prob = df_svm_fe$.pred_1,
  folder = "plot_feli_svm",
  height = 250,
  width = 250
)

svm_r_clp = classifierplots::classifierplots(df_svm_r_fe$INFEC_after_diag, df_svm_r_fe$.pred_1)
svm_r_clp

dir.create("plot_feli_svm_r")
classifierplots::classifierplots_folder(
  test.y=df_svm_r_fe$INFEC_after_diag,
  pred.prob = df_svm_r_fe$.pred_1,
  folder = "plot_feli_svm_r",
  height = 250,
  width = 250
)

xgb_clp = classifierplots::classifierplots(df_xgb_fe$INFEC_after_diag, df_xgb_fe$.pred_1)
xgb_clp

dir.create("plot_feli_xgb")
classifierplots::classifierplots_folder(
  test.y=df_xgb_fe$INFEC_after_diag,
  pred.prob = df_xgb_fe$.pred_1,
  folder = "plot_feli_xgb",
  height = 250,
  width = 250
)

nb_clp = classifierplots::classifierplots(df_nb_fe$INFEC_after_diag, df_nb_fe$.pred_1)
nb_clp

dir.create("plot_feli_nb")
classifierplots::classifierplots_folder(
  test.y=df_nb_fe$INFEC_after_diag,
  pred.prob = df_nb_fe$.pred_1,
  folder = "plot_fe_nb",
  height = 250,
  width = 250
)

mlp_clp = classifierplots::classifierplots(df_mlp_fe$INFEC_after_diag, df_mlp_fe$.pred_1)
mlp_clp

dir.create("plot_fe_mlp")
classifierplots::classifierplots_folder(
  test.y=df_mlp_fe$INFEC_after_diag,
  pred.prob = df_mlp_fe$.pred_1,
  folder = "plot_fe_mlp",
  height = 250,
  width = 250
)

bag_clp = classifierplots::classifierplots(df_bag_fe$INFEC_after_diag, df_bag_fe$.pred_1)
bag_clp


dir.create("plot_fe_bag")
classifierplots::classifierplots_folder(
  test.y=df_bag_fe$INFEC_after_diag,
  pred.prob = df_bag_fe$.pred_1,
  folder = "plot_fe_bag",
  height = 250,
  width = 250
)
dec_clp = classifierplots::classifierplots(df_dec_fe$INFEC_after_diag, df_dec_fe$.pred_1)
dec_clp

dir.create("plot_fe_dec")
classifierplots::classifierplots_folder(
  test.y=df_dec_fe$INFEC_after_diag,
  pred.prob = df_dec_fe$.pred_1,
  folder = "plot_fe_dec",
  height = 250,
  width = 250
)

mars_clp = classifierplots::classifierplots(df_mars_fe$INFEC_after_diag, df_mars_fe$.pred_1)
mars_clp

dir.create("plot_fe_mars")
classifierplots::classifierplots_folder(
  test.y=df_mars_fe$INFEC_after_diag,
  pred.prob = df_mars_fe$.pred_1,
  folder = "plot_fe_mars",
  height = 250,
  width = 250
)

cll_st_clp = classifierplots::classifierplots(df_cll_st_fe$INFEC_after_diag, df_cll_st_fe$.pred_1)
cll_st_clp

dir.create("plot_feli_cll_st")
classifierplots::classifierplots_folder(
  test.y=df_cll_st_fe$INFEC_after_diag,
  pred.prob = df_cll_st_fe$.pred_1,
  folder = "plot_feli_cll_st",
  height = 250,
  width = 250
)
