---
title: "MPM RWE with feature enginering"
...
saveRDS(lr_res_fe, "/users/home/candwei/lr_res_fe")
```
...
saveRDS(rf_res_fe, "/users/home/candwei/rf_res_fe")
```
...
svm_grid_fe$degree = as.integer(svm_grid_fe$degree)
...
saveRDS(svm_res_fe, "/users/home/candwei/svm_res_fe")
...
saveRDS(svm_r_res_fe, "/users/home/candwei/svm_r_res_fe")
```
We can print the metrics from the model training using the collect_metrics command.
```{r}
svm_r_res_fe %>% 
...
saveRDS(xgb_res_fe, "/users/home/candwei/xgb_res_fe")
...
saveRDS(mlp_res_fe, "/users/home/candwei/mlp_res_fe")
...
saveRDS(nb_res_fe, "/users/home/candwei/nb_res_fe")
...
saveRDS(bag_res_fe, "/users/home/candwei/bag_res_fe")
...
saveRDS(dec_res_fe, "/users/home/candwei/dec_res_fe")
```
...
# Last fit
lr_last_fit_fe <- last_fit(lr_final_workflow_fe, split = df_split)
lr_last_fit_fe %>% collect_metrics()
```
...
# Last fit
rf_last_fit_fe <- last_fit(rf_final_workflow_fe, split = df_split)
rf_last_fit_fe %>% collect_metrics()
```
...
# Last fit
svm_last_fit_fe <- last_fit(svm_final_workflow_fe, split = df_split)
svm_last_fit_fe %>% collect_metrics()
```
...
# Last fit
svm_r_last_fit_fe <- last_fit(svm_r_final_workflow_fe, split = df_split)
svm_r_last_fit_fe %>% collect_metrics()
```
...
# Last fit
xgb_last_fit_fe <- last_fit(xgb_final_workflow_fe, split = df_split)
xgb_last_fit_fe %>% collect_metrics()
```
...
# Last fit
mlp_last_fit_fe <- last_fit(mlp_final_workflow_fe, split = df_split)
mlp_last_fit_fe %>% collect_metrics()
```
...
# Last fit
bag_last_fit_fe <- last_fit(bag_final_workflow_fe, split = df_split)
bag_last_fit_fe %>% collect_metrics()
```
...
# Last fit
nb_last_fit_fe <- last_fit(nb_final_workflow_fe, split = df_split)
nb_last_fit_fe %>% collect_metrics()
```
...
# Last fit
dec_last_fit_fe <- last_fit(dec_final_workflow_fe, split = df_split)
dec_last_fit_fe %>% collect_metrics()
```
...
cll_st_fe <- 
  stacks() %>%
  add_candidates(lr_res_fe) %>%
  add_candidates(rf_res_fe) %>%
  add_candidates(svm_res_fe) %>%
  add_candidates(svm_r_res_fe) %>%
  add_candidates(xgb_res_fe) %>%
  add_candidates(nb_res_fe) %>%
  add_candidates(mlp_res_fe) %>%
  add_candidates(bag_res_fe) %>%
  add_candidates(dec_res_fe) %>%
  add_candidates(mars_res_fe) %>%
  blend_predictions() %>%
  fit_members()

saveRDS(cll_st_fe, "/users/home/candwei/cll_st_fe")
```
