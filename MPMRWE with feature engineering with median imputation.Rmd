---
title: "MPMRWE-withFE-median-impute"
...
saveRDS(lr_res_femi, "/users/home/candwei/lr_res_femi")
```
...
saveRDS(rf_res_femi, "/users/home/candwei/rf_res_femi")
```
...
svm_grid_femi$degree = as.integer(svm_grid_femi$degree)

svm_res_femi <- 
  tune_grid(
    object = svm_wflow_femi, 
    resamples = df_val, 
    grid = svm_grid_femi,
    control = ctrl_grid,
    metrics = metric
...
saveRDS(svm_res_femi, "/users/home/candwei/svm_res_femi")
...
svm_r_res_femi <- 
  tune_grid(
    object = svm_r_wflow_femi, 
    resamples = df_val, 
    grid = svm_r_grid_femi,
    control = ctrl_grid,
    metrics = metric
  )

saveRDS(svm_r_res_femi, "/users/home/candwei/svm_r_res_femi")
```
We can print the metrics from the model training using the collect_metrics command.
```{r}
svm_r_res_femi %>% 
...
saveRDS(xgb_res_femi, "/users/home/candwei/xgb_res_femi")
...
saveRDS(mlp_res_femi, "/users/home/candwei/mlp_res_femi")
...
saveRDS(nb_res_femi, "/users/home/candwei/nb_res_femi")
...
saveRDS(bag_res_femi, "/users/home/candwei/bag_res_femi")
...
saveRDS(dec_res_femi, "/users/home/candwei/dec_res_femi")
```
...
mars_grid_femi$num_terms = as.numeric(mars_grid_femi$num_terms)
mars_grid_femi$prod_degree = as.numeric(mars_grid_femi$prod_degree)


mars_res_femi <- 
  tune_grid(
    object = mars_wflow_femi, 
    resamples = df_val, 
    grid = mars_grid_femi,
    control = ctrl_grid,
    metrics = metric
...
saveRDS(mars_res_femi, "/users/home/candwei/mars_res_femi")
```
...
cll_st_femi <- 
  stacks() %>%
  add_candidates(lr_res_femi) %>%
  add_candidates(rf_res_femi) %>%
  add_candidates(svm_res_femi) %>%
  add_candidates(svm_r_res_femi) %>%
  add_candidates(xgb_res_femi) %>%
  add_candidates(nb_res_femi) %>%
  add_candidates(mlp_res_femi) %>%
  add_candidates(bag_res_femi) %>%
  add_candidates(dec_res_femi) %>%
  add_candidates(mars_res_femi) %>%
  blend_predictions() %>%
  fit_members()

saveRDS(cll_st_femi, "/users/home/candwei/cll_st_femi")
```
